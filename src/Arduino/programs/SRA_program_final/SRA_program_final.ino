...
/**
*This time runs every loop iteration, and checks to see if
*we need to do something
*/
void timer()
{
  unsigned long timePast = currCheckTm - lastCheckTm;
  if(timePast >= timerInterval)//if its bigger than the interval its time to get to work!
  {
    if(enProt1F == true)
    {
      enProtocal1(1);
    }
    else if(enProt2F == true)
    {
      //protocol 2
    }
  }
...
void checkpwCT()
{
  ...
  */
  ...
  /**
  *Why are you adding lastInterval, and interval?
  *So since they are unsigned longs they can be negative. So I can only add them.
  *The idea is if lastInterval is say 15 minutes, and interval is also 15 minutes,
  *then together they will be double lastInterval. Im adding some wiggle room by saying
  * lastInterval+ interval can be +/- 5 minutes of lastInterval*2.
  */
  if((lastInterval + interval) > ((lastInterval*2) - 300000) && //floor. 
    (lastInterval + interval) < ((lastInterval*2) + 3000000)) //cieling
  {
 ...
 * It will shut certain parts or turn them back on
 */
int enProtocal1(int command)
{
...
/**
*This will start protocol 1
*/
int enProtocal1Start(unsigned long interval)
{
  //is this already on?
  if(enProt1F)
  {
    //we are changing the stuff
    enProtocal1(0);
